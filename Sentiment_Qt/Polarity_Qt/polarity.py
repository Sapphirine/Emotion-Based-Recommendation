		...
		while (i < N):
			...
			while ( j > i ):
				...
				if literal in self.dictionary:
					...
					if is_single_token:
						original_token_taggin = sentence[original_position][2]
						tagged_expression[2].extend(original_token_taggin)
...
for (dirpath, dirnames, filenames) in walk(yelp_path):
		...
		dict_tagged_sentences = dicttagger.tag(pos_tagged_sentences)
                ...
                out.write( repr(sentiment_score(dict_tagged_sentences)) + '\n' )
                out.write( shop + '\n' )
